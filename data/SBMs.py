...
class load_SBMsDataSetDGL(torch.utils.data.Dataset):
    ...
    def _prepare(self):
        ...
        for data in self.dataset:
            ...
            # g.edata['feat'] = torch.rand(g.number_of_edges(), edge_feat_dim)
...
def positional_encoding(g, pos_enc_dim):
    ...
    A = torch.tensor(A.toarray()).float().to("cuda")
    N = torch.diag(g.in_degrees().clip(1) ** -0.5).float().to("cuda")
    L = torch.eye(g.number_of_nodes()).to("cuda") - N.mm(A).mm(N)
    ...
    g.ndata['pos_enc'] = EigVec[:,1:pos_enc_dim+1].cpu().float() 
